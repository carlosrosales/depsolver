...
*                                                                             *
* forcePost is free software; you can redistribute it and/or modify           *
* it under the terms of the GNU General Public License as published by        *
* the Free Software Foundation; either version 2 of the License, or           *
* (at your option) any later version.                                         *
*                                                                             *
* forcePost is distributed in the hope that it will be useful,                *
* but WITHOUT ANY WARRANTY; without even the implied warranty of              *
* MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the               *
* GNU General Public License for more details.                                *
*                                                                             *
* You should have received a copy of the GNU General Public License           *
* along with forcePost; if not, write to the Free Software                    *
...
if(argc == 2 && strcmp(argv[1],"-h") == 0){
    ...
    printf("\n\nCorrect syntaxix is:\n\n\t./forcePost nx ny nxi nyi\n");
    errorHandler("Type './forcePost -h' for help");
}
else{
    NX = atoi(argv[1]);
    NY = atoi(argv[2]);
...
if((fp = fopen("force.dat","r")) == NULL) errorHandler("Unable to open input file 'force.dat'");
...
/* OPEN OUTPUT FILES */
if((fX = fopen("X.dat","w")) == NULL) errorHandler("Unable to open output file 'X.dat'");
if((fY = fopen("Y.dat","w")) == NULL) errorHandler("Unable to open output file 'Y.dat'");
if((fXX = fopen("XX.dat","w")) == NULL) errorHandler("Unable to open output file 'XX.dat'");
if((fYY = fopen("YY.dat","w")) == NULL) errorHandler("Unable to open output file 'YY.dat'");
if((fXXI = fopen("XXI.dat","w")) == NULL) errorHandler("Unable to open output file 'XXI.dat'");
if((fYYI = fopen("YYI.dat","w")) == NULL) errorHandler("Unable to open output file 'YYI.dat'");
if((fFX = fopen("FX.dat","w")) == NULL) errorHandler("Unable to open output file 'FX.dat'");
if((fFY = fopen("FY.dat","w")) == NULL) errorHandler("Unable to open output file 'FY.dat'");
if((fFZ = fopen("FZ.dat","w")) == NULL) errorHandler("Unable to open output file 'FZ.dat'");
if((fFXX = fopen("FXX.dat","w")) == NULL) errorHandler("Unable to open output file 'FXX.dat'");
if((fFYY = fopen("FYY.dat","w")) == NULL) errorHandler("Unable to open output file 'FYY.dat'");
...
else errorHandler("No constant plane in input file!!");
