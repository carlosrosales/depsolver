...
* on the dielectric interface if required and stores it in file               *
...
#include "postProcessCol_tria3.h"
                         ...
                         double **xCols, double *vB)
{
    ...
    for(i = 0; i < nInternalPoints; i++){
        ...
        if(ANALYSIS == 0){
            if(nPointInside) potential_tria3(Xin,mNodes,mElems,vB,mPot);
            fprintf(fP,"%le\t%le\t%le\t",Xin[0],Xin[1],Xin[2]);
            fprintf(fP,"%le\t%le\n",mPot[0],mPot[1]);
        }
        else if(ANALYSIS == 1){
            if(nPointInside) field_tria3(Xin,mNodes,mElems,vB,mE);
    ...
    if(ANALYSIS == 3 || ANALYSIS == 4){
        fprintf(file_log,"\n\tpostProcess_tria3(): calling forceMST_tria3() ...");
        ...
        fprintf(fF,"%le\t%le\t%le\t",Xcm[0],Xcm[1],Xcm[2]);
        fprintf(fF,"%le\t%le\t%le\n",Fcm[0],Fcm[1],Fcm[2]);
    ...
    /* Calculate Fdep at particle centre using multipolar method if required */
        ...
        fprintf(file_log,"\n\tpostProcessCol_tria3(): calling ");
        fprintf(file_log,"forceMultipole_tria3() ...");
        nShape = 0;
        switch(ANALYSIS){
        ...
        fF = fopen("force-dp.dat","w");
        fprintf(fF,"#Xcm\t\tYcm\t\tZcm\t\tFXcm\t\tFYcm\t\tFZcm\n");
        for(i = 0; i < nFPoints; i++){
            Xeval[0] = XF[i][0];
            Xeval[1] = XF[i][1];
            Xeval[2] = XF[i][2];
            if(!nShape) forceMultipole_tria3(nOrder,R,Xeval,mNodes,mElems,vB,vMatParam,vProbParam,Fcm);
