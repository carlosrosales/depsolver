...
int main()
{
    ...
    if((ANALYSIS > 4) && (ANALYSIS < 10)){
        fscanf(fIn,"%d %le %s",&nFPoints,&axis[0],cForcePoints);
        fAux = fopen(cForcePoints,"r");
        XF = doubleMatrix(nFPoints,3,0);
        for(i = 0; i < nFPoints; i++)
            fscanf(fAux,"%d %le %le %le",&nBuffer,&XF[i][0],&XF[i][1],&XF[i][2]);
        fclose(fAux);
    }
    
    /* Dipolar approximation for ellipsoid */
    else if(ANALYSIS == 10 || ANALYSIS == 11){
        fscanf(fIn,"%d %le %le %le %s",&nFPoints,&axis[0],&axis[1],&axis[2],cForcePoints);
        fAux = fopen(cForcePoints,"r");
        XF = doubleMatrix(nFPoints,3,0);
        for(i = 0; i < nFPoints; i++)
            fscanf(fAux,"%d %le %le %le",&nBuffer,&XF[i][0],&XF[i][1],&XF[i][2]);
    ...
    if(nElemType == 5){
        fprintf(file_log,"\ndepSolver(): calling formMatrix_tria3() ...");
        ...
        fprintf(file_log,"\ndepSolver(): calling formMatrix_tria6() ...");
        if(strcmp(solver,"GMRES") == 0)
    ...
    /* Solve linear system */
    if(strcmp(solver,"GMRES") == 0){
        fprintf(file_log,"\ndepSolver(): calling solverGMRES() ...");
        solverGMRES_el(preCond,nInit,mA,vB);
    }
    else{
        fprintf(file_log,"\ndepSolver(): calling gaussBksb() ...");
    ...
    if(nElemType == 5){
        fprintf(file_log,"\ndepSolver(): calling postProcess_tria3() ...");
        if(nCols > 0){
        ...
        fprintf(file_log,"\ndepSolver(): calling postProcess_tria6() ...");
        if(nCols > 0){
