...
* SubA, for the IBEM AC electrostatic case. Uses symmetry to reduce storage.  *
                        ...
                        double **mBC, double *vB)
{
    ...
    int i, interfaceID, j, k, mat1, mat2, currentNode, SinNode;  
    ...
    for(i = 0; i < nNodes; i++){
        ...
        for(j = 0; j < nElems; j++){
            ...
            /* Potential given */
            if(vBCType[i] == 1){
                if(!SinNode) intG_tria6(X,Xeval,SubA);
                else intSingularG_tria6(SinNode,X,Xeval,SubA);

                /* Assign contributions to the right places in the matrix */
                for(k = 0; k < NODES_IN_ELEM; k++)
                    mA[i][mElems[j][k] - 1] += C*SubA[k];
            }
        
            /* Dielectric interface */
            else{
                if(!SinNode) intH_tria6(FLAG,X,Xeval,SubA,normal);
                ...
                for(k = 0; k < NODES_IN_ELEM; k++){
                    ...
                    mA[i][currentNode + nNodes] += B[interfaceID]*SubA[k];
                }
            }
        }
