#! /bin/sh
#
# Installation script for depSolver v2.0
# Default compiler is gcc with optimization level O3 flags.
# 
# v1.0                                     13.06.2008 - Carlos Rosales Fernandez
...
WORKDIR=$(pwd)
LOG="$WORKDIR/depSolver_install.log"
ERR="$WORKDIR/depSolver_install.err"

MSG="Starting installation...\n\n"
printf "$MSG" && printf "$MSG" >> $LOG
MSG="Working Directory : $WORKDIR\n\n"
printf "$MSG" && printf "$MSG" >> $LOG
MSG="Compiling depSolver binaries.... "
printf "$MSG" && printf "$MSG" >> $LOG
MSG="Done.\n"
...
if test -s "$ERR"
then
  MSG="\n*** WARNING: Errors detected. Check depSolver_install.err. ***\n"
  printf "$MSG" && printf "$MSG" >> $LOG
  MSG="***          Installation may be incomplete.               ***\n\n"
  printf "$MSG" && printf "$MSG" >> $LOG
else
  MSG="\ndepSolver v2.0 installation completed successfully!\n\n"
  printf "$MSG" && printf "$MSG" >> $LOG
  printf "For details see depSolver_install.log.\n\n"
fi
